...
//new_index is new_query left_shift the value of index
  ...
  new_index = shift_work(new_query,index, num_slots);
  cout << index << endl;
  //out(new_index);
  //out(new_query);
  ...
  new_query.resize(num_slots);
  Ctxt new_PIR_index(publicKey);
  new_index.resize(num_slots);

  ea.encrypt(new_PIR_query, publicKey, new_query);
  ea.encrypt(new_PIR_index, publicKey, new_index);

...
//  vector<vector<long> > outputTable;
//  vector <long> outputTable_row;
  ...
  for(int i=0 ; i<TABLE_SIZE_ROW ; i++){
    for(int j=0 ; j<TABLE_SIZE_COL ; j++){
      if(i==0 && j==0) inputTable_row.push_back(1);
      else if(i==1 && j==0) inputTable_row.push_back(1);
      else {
        int s = (generator()%10+2);
        inputTable_row.push_back(s);
      }
    }
